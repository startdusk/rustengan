...
impl<Payload> Message<Payload> {
    ...
    where
        Payload: Serialize,
    {
        serde_json::to_writer(&mut *output, &self).context("serialize response to message")?;
...
where
    P: DeserializeOwned + std::marker::Send + 'static,
    N: Node<S, P, IP>,
    IP: std::marker::Send + 'static,
{
    ...
    let mut node: N =
        Node::from_init(init_state, init, tx.clone()).context("node initlization failed")?;
    let reply = Message {
        src: init_msg.dst,
        dst: init_msg.src,
        body: Body {
    ...
    let jh = std::thread::spawn(move || {
        ...
        for line in stdin.lines() {
            let line = line.context("Maelstrom line from STDIN could not be read")?;
